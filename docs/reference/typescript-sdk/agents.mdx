...
description: "The management API client on the portable @opencomputer/sdk/agents subpath"
---
...
| `invalid_response` | The call succeeded but its body is not JSON or does not match the documented shape of the route. `status` is the HTTP status; the message names the route and the first field that did not match. |

## Validation

Every response is checked against the documented shape of its route before
the method returns: the fields the [API reference](/agents/api) lists, with
their types. A field the documentation does not list passes through as it
came, so an addition to the API never fails a call. Application data, that
is a session's `result.data`, a turn's `payload`, and a tool call's `input`
and `output`, is checked only as a JSON value; what it contains is yours to
validate.
