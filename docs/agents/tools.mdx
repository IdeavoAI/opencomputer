...
| `result` | no | `true` marks this tool as the agent's [result tool](#the-session-result); one per agent |
...
The build enforces two rules: an agent declares at most one result tool,
and a result tool declares `output`. The schema is recorded with the
deployment, and every value the tool returns is validated against it.
