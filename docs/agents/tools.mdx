...
confirmed replaces the computer. A session whose `executionMode` is
`microvm` still cancels immediately on interrupt, without waiting for its
commands to be confirmed stopped, until sessions in that mode settle the
same way.
