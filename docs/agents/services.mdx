...
Omitting `label` here connects it as `default`, which is the label the examples
...
| --- | --- | --- |
| `404 connection_not_found` | No account under **that label** — often a call with no `label` while the account was connected as something else | Pass the matching `label`, or connect one (step 1) |
