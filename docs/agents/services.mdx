...
**Disconnecting one.** The CLI takes the label; the API takes the connection id
from the listing above, and the service rather than the provider.
...
| A connection stays `pending` | Nobody has opened the link yet, or it expired | `opencomputer connection list` re-checks pending accounts with the provider, so its answer is current |
