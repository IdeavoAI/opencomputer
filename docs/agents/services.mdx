...
\* Drive and Sheets are recognised by the SDK but are not enabled on the
platform yet — connecting one fails until an auth config exists for it. Gmail,
Calendar and GitHub work today.
