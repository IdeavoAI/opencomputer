...
# The listing reports a stored status. To find out whether a `pending`
# account has since been authorized, ask the status route — it is the one
# that re-checks with the provider, and records the answer.
curl "$OC_API_URL/api/managed-agents/connections/google/status?service=gmail&label=support" \
  -H "x-api-key: $OPENCOMPUTER_API_KEY"
```
</CodeGroup>

<Note>
`status` in the listing is stored, not live. An account someone authorized a
minute ago can still read `pending` there until something re-checks it. The CLI
does that for you; over the API, call the status route above. Agents are
unaffected — `listServices()` reconciles before it answers.
</Note>
...
| A connection stays `pending` | Nobody opened the link, it expired — or the listing's stored status is stale | `opencomputer connection list` re-checks for you; over the API call the status route |
