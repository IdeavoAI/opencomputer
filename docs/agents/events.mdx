...
A model call that fails in flight is retried inside the turn before the
turn fails: provider errors and rate limits on the runtime's own schedule,
and a transport failure or a response stream that breaks once more. Each
retry appears in the log as a `runtime.log` milestone with
