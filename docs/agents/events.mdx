...
follows and the next turn starts. A session whose `executionMode` is
`microvm` does not pass through `stopping`: an interrupt cancels its turn
immediately, without waiting for the turn's commands, and the
`turn.cancelled` event carries `reason` alone, none of the settlement
...
React hook's [`turns`](/agents/react#activity) shows those calls unnamed and
