...
A terminal turn event, `turn.completed`, `turn.failed` or `turn.cancelled`,
settles every tool call of the turn that has no completion of its own. The
log records a `tool.completed` or `tool.failed` per call while the runtime is
connected; a call without one ended with its turn, and the turn's outcome is
the call's outcome.
