...
| `tool.completed` | The tool returned | `tool`, `callId`, `title`, `output`; `result: true` when the output became the session's result |
...
is recorded with `result: true`; its `output` is the validated value as JSON
text, the same value `GET /sessions/<id>` returns parsed as `result.data`.
The session writes this event when it commits the result, before the model
sees the call succeed, and there is one per call.
