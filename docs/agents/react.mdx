...
[management client](/reference/typescript-sdk/managed-agents) creates the
...
`send` resolves once the platform has admitted the input as a turn,
...
the entry its `tool.started` created rather than adding one. When a turn ends,
every call still `running` is settled with the turn: `completed` when the turn
completed, `failed` when it failed, `cancelled` when it was interrupted. A
settled turn therefore never shows a running call. An activity timeline
renders `turns` directly:
...
| `send(text, options?)` | Starts a turn; resolves with `{ sessionId, turnId, status }` or rejects with a `SendError`. `options` carries `idempotencyKey` and `payload`. In create mode the first call creates the session |
