...
code. Resolve an allowed deployment when composing the submission, then
retain its ID and a creation key for retries:
...
Here `deploymentId` and `taskId` belong to one submission. Validate the
requested deployment against the user's permitted project, environment and
agent. Retry an uncertain create with the same parameters and key; resolving
an alias again could select a newer deployment. Creating a session and
sending its first turn are separate calls with separate keys.
...
`stop()` requests an interrupt without spending a model turn. It returns
`Promise<void>` and reports request failures through `error`; a resolved
promise is **not** confirmation that execution has stopped. Watch the running
...
| `stop()` | Request an interrupt; errors appear in `error`, settlement in `turns` |
