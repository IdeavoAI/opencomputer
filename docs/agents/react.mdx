...
never shows a running call. Tool events recorded before the current fields
were introduced keep the shape they were recorded with, so `turns` shows
those calls unnamed and without output; `messages` is unaffected. An
activity timeline renders `turns` directly:
