...
[TypeScript client](/reference/typescript-sdk/managed-agents) on
`@opencomputer/sdk/managed-agents` mirrors these routes one to one.
...
Any other parameter is `400 invalid_query`, as is a malformed value. A
project-scoped key lists its own project; naming another is
...
Rows are ordered by `createdAt`, newest first, then by `id`, and the order
never changes: a session that is updated does not move, so a traversal with
`cursor` sees every row exactly once even while sessions are running. To show
recent activity first, sort the page you hold by `updatedAt`. `nextCursor` is
`null` on the last page. A session appears in the list as soon as its create
call has returned.
...
model turn. The response shows the session as `stopping` with the turn still
`running`: the model loop is stopped, and every command the turn had started
on its computer is stopped before the turn settles. The turn then settles as
`cancelled` with reason `interrupted`, the session returns to `idle` and the
next queued turn starts. A command that cannot be confirmed stopped is
resolved by replacing the computer, and the turn settles once the
replacement is confirmed; the turn's `turn.cancelled` event records how the
stop settled ([Session events](/agents/events#turns)). An idle session is
returned unchanged.
...
| `idempotencyKey` | Optional. Identifies this request: a repeat returns the existing turn, a different request under the same key is refused (below). Without one every request starts a turn. |
...
1. Resolve the deployment once, where the submission is composed (for
   example when the form opens), and pin it: `POST /sessions` with
   `Idempotency-Key: <submission-id>`, the `deploymentId` and `environment`.
   A retry then carries the deployment the submission was composed against,
   even if the alias has moved since.
2. `POST /sessions/<id>/turns` with `idempotencyKey: <submission-id>/start`,
   the text and the `payload`.

Keep the submission (its id, the pinned deployment, the text and the
payload) until the turns call answers, with `duplicate: false` or `true`.
...
[TypeScript client](/reference/typescript-sdk/managed-agents) does this
