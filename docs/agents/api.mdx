...
| Find by target | `GET /channels/slack/setups` | `?agentId=&alias=&channelId=` | `200` with `{ setup }`, `null` when nothing is in progress |
...
in code; omit it for an agent whose deployment declares none. `requestKey` is
...
| `slack_setup_active` | Another setup is in progress for the target; find it by target and continue it |
...
| `slack_provider_unavailable` | Slack did not answer; submit or authorize again |
| `slack_creation_uncertain` | See phase `creation_uncertain` |
| `slack_exchange_uncertain`, `slack_authorization_denied`, `slack_authorization_expired` | The installation did not complete; authorize again |
...
redirected to the project's Connections tab with `slack=<result>` and
`setup=<id>` in the query. `result` is `connected` or one of
