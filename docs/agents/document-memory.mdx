...
neither is created as a side effect of the other. The SDK and CLI can create
the document and then the session in one call. These are separate requests;
the document remains if session creation fails.
...
- The session is created with an `Idempotency-Key` derived from
  ...
  and bindings returns the existing session; anything else under that key
  fails with code `idempotency_key_reused`. `access` (default `read-write`),
  `memory` for further bindings and `source` are passed through.

The helper resolves the agent's environment alias on each attempt. A
redeploy can therefore make a retry conflict even if the first attempt
created the session. Do not change the key to recover a lost response: that
would create another session. For retries across redeploys, use separate
document and session calls with a retained deployment ID, as described below.
...
/api/managed-agents/sessions` with the document binding, a retained
`deploymentId`, `environment` and `Idempotency-Key`; `201` created the
session, `200` returned the one that key had already created. Keep that
envelope unchanged for retries. See [session creation](/agents/api#create)
for the body and the
[Serverless Agents client reference](/reference/typescript-sdk/agents#start-a-session-on-a-memory-document).
